...
public class Main {
    ...
    private static void readConfig(String configFilePath) {
        ...
        politenessDelay = Integer.parseInt(config.getProperty("politenessDelay", "100"));
    }
    ...
    private static void startThreads(String initFilePath) throws Exception {
        ...
        for (int i = 0; i < indexBuildingThreads.size(); ++i) {
            new Thread(indexBuildingThreads.get(i), "IBT" + i).start();
        }

        for (int i = 0; i < crawlers.size(); ++i) {
            new Thread(crawlers.get(i), "CT" + i).start();
        }
