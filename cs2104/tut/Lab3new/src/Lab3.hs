    ...
    case dd of
       ...
       Right d -> 
          let x = d !! 1
              ...
              get_extra s = foldl (+) 0 (fmap (\x -> if (x == "") then 0.2 else 0) s)
              get_bonus s = (if ((s ! 12) == "None of the Given Answers") then 1 else 0) +
                  (if ((s ! 16) == "None of the Given Answers") then 1 else 0)
              get_score cur s ans = (read cur::Double) + (get_extra ans) + (get_bonus ans)
              scores = fmap (\(a, b, c, d) -> (a, (get_score b c d))) stud_res
              nth :: Ord t => [t] -> Int -> t
              nth (x:xs) n
                | k == n = x
                | k > n = nth ys n
              ...
              medianMay :: (Fractional a, Ord a) => [a] -> a
              medianMay xs
                | even n = (nth xs (div n 2) + nth xs (div n 2 - 1)) / 2.0
                | otherwise = nth xs (div n 2)
                where
                  n = length xs
               
          in (writeFile "adjusted_mark_Quiz1.txt" (
                "===================\n" ++ 
                "Score (Sorted by ID):\n" ++ 
                id (foldl (\b a -> b ++ a ++ "\n") "" $ map (\(a, b) -> a ++ ", " ++ show b) $ sort scores) ++
                "\nScore (Sorted by Score):\n" ++
                id (foldl (\b a -> b ++ a ++ "\n") "" $ map (\(a, b) -> a ++ ", " ++ show b) $ 
                    sortWith (\(a, b) -> (b, a)) scores) ++
                "\nMin:"++
                show (minimum $ map snd scores) ++
                ",Max:"++
                show (maximum $ map snd scores) ++
                ",Median:"++
                showFFloat (Just 1) (medianMay $ map snd scores) "" ++
                ",Average:" ++
                showFFloat (Just 1) ((sum $ (map snd scores)) / (fromIntegral (length scores))) ""
                ))
    {-
          (putStrLn (show (d !! 0))
