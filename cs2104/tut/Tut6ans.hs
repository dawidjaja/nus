...
For participation mark, you can choose to submit either Tut5 or Tu6 or both.
Please send in to folder for Tut 5/6.
...
expr = chainl1 term mulop --addop            -- x+y-z+...
term = chainl1 factor addop -- mulop          -- x*y/z*...
...
term = chainr1 factor mulop          -- x*y/z*...
chainr1
  :: Stream s m t =>
...
expr :: SParsec Expr
expr = chainr1 expr1 powop            -- x+y-z+...

expr1 :: SParsec Expr
expr1 = chainl1 term addop            -- x+y-z+...
...
onetoken = 
       ...
       <|> (parsecMap (\x -> WPower) $ reservedOp "^")
       <|> (parsecMap (\x -> WDiv) $ reservedOp "/")
       <|> (parsecMap (\x -> WOpen) $ reservedOp "(")
       <|> (parsecMap (\x -> WClose) $ reservedOp ")")
       <|> (parsecMap (\x -> WEq) $ reservedOp "=")
       <|> (parsecMap (\x -> WLet) $ reserved "let")
       <|> (parsecMap (\x -> WIn) $ reserved "in")
       <|> (parsecMap (\x -> WInt x) $ integer)

mtokens = many1 (whiteSpace >> onetoken)
