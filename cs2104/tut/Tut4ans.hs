...
{-
   ...
   For this week, we will not have any presentations.
   We will just go through Tut4, and also run through
   questions you may have for a sample quiz.
   Also, try complete this tutorial by yourself and submit in 
   LumiNUS for participation mark by 19Sep2020. It will NOT be graded.
-}
...
lis a (x:xs) = 
    let b = lis a xs in
    let c = if a < x then (1 + (lis x xs))
                     else 0 in
    max c b
    

longest x = lis 0 $ filter (\y -> y >= 0) x
