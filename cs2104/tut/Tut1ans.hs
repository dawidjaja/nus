...
{-
After tutorial note:
As Maybe is an instance of Ord, and 
  max :: Ord a => a -> a -> a

One can also write maxlist3a, as follows:

maxlist3a xs =
  case xs of
    [] -> Nothing
    (x:ys) -> max (Just x) (maxlist3a ys)

However, in such an encoding, Nothing corresponds to the
smallest possible value (negative infinity), and
max function works just fine.

However, if we were to try determine the smallest value
from a list, we would not be able to use the min
method since:

  min Nothing (Just x) = Nothing

when we would expect instead:

  min Nothing (Just x) = (Just x)
  
Hence, using explicit manipulation via case
on Maybe type is still the more general way to
