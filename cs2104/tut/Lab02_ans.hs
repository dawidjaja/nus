...
product2 xs ys = [(x, y) | y <- ys, x <- xs, x * y > x + y]
{-|
   (c)
...
divisorProd xs ys = [x `div` y | y <- ys, x <- xs, y /= 0]
{-|
   Q3:
...
countNums1 xs = foldl (\(countpos,countneg,count) x -> if x>0 then (countpos+1,countneg,count) else if x<0 then (countpos,countneg+1,count) else (countpos,countneg,count+1)) (0,0,0) xs

countNums2 :: [Int] -> (Int, Int, Int)
countNums2 xs = let 
    countpos = length (filter (\x -> x > 0) xs)
    countneg = length (filter (\x -> x < 0) xs)
    count = length (filter (\x -> x == 0) xs)
  in (countpos,countneg,count)
{-|
...
-}
leftMost :: Tree a -> a
leftMost t = foldTree  (\v -> v) (\v l r -> l) t
{-|
   (c)
...
mirrorTree t = foldTree (\v -> Leaf v) (\v l r -> Node v r l) t
t4 :: Tree Char
...
addSize t = let getSize a = case a of
                     Leaf v -> fst v
                     Node v l r -> fst v
               in foldTree (\x -> Leaf (1, x)) (\v l r -> Node (1 + (getSize l) + (getSize r), v) l r) t
{-|
   (e)
...
checkMaxHeap t = snd (foldTree (\v ->(v,True)) (\v (l,accl) (r,accr) -> (v, accl && accr && v>=l && v>=r)) t)
...
showTree2 =
  let
    aux indent (Leaf v) = indent ++ "Leaf " ++ show v ++ "\n"
    aux indent (Node v leftTree rightTree) =
      indent ++ "Node " ++ show v ++ "\n" ++
        (aux (indent ++ "  ") leftTree) ++
        (aux (indent ++ "  ") rightTree)
  in
    aux ""
...
showListNum separator xs = 
  "[" ++ L.intercalate separator (map (\(num, x) -> "(" ++ (show num) ++ ")" ++ show(x)) (addNum xs) ) ++ "]"
